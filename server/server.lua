    ...
    return {
        ...
        face = {
            ...
            overlays = {
                ...
                -- Bart: Opacity nur setzen wenn Style > 0 explizit gesetzt – index=0 + opacity=1 ist ESX-Default ohne Bart
...
AddEventHandler('esx_skin:save', function(skin)
    ...
    -- Gesicht-Daten (HeadBlend, Overlays, Bart, Hautfarbe, …) IMMER aus dem Server-Cache
    -- übernehmen. Der Kleidungsshop darf das Gesicht NIEMALS verändern:
    -- GTA V setzt HeadBlend/Overlays beim Kleidungswechsel intern zurück, sodass die vom
    -- Shop mitgeschickten Face-Daten immer falsch/resettet sind.
    if skinCache[identifier] then
        local cached = json.decode(skinCache[identifier])
        if cached and cached.face then
            awData.face = cached.face
            dbg('esx_skin:save | Face-Daten aus Cache erzwungen (Kleidungsshop darf Gesicht nicht ändern)')
        end
    else
        dbg('esx_skin:save | WARNUNG: kein Server-Cache für %s – Face-Daten aus Shop-Daten (Fallback)', identifier)
    end
